...
import { ModalModeProps, Cafeteria } from '@/app/types/types';
import { addCafeteria, getLiteEmployees2, updateCafeteria } from '@/app/utils/funcs';
...
const AddCafeteriaModal: React.FC<ModalModeProps<Cafeteria>> = (props) => {
  ...
  const { register, handleSubmit, reset } = useForm<Cafeteria>({
  ...
  useEffect(() => {
    if (props.mode === OperationMode.update) {
      const assignedEmpls = (props.initialData?.cafeteriaEmployee as Array<{ employeeId: string, firstName: string, lastName: string, role: string }>).map((employee) => employee.employeeId);
      const initialData: Cafeteria = {
        name: props.initialData?.name || "",
        location: props.initialData?.location || "",
        description: props.initialData?.description || "",
        cafeteriaEmployee: assignedEmpls?.join(',') || "",
      }
      reset(initialData);
      setTimeout(() => {
  ...
  async function handleAddCafeteria(data: Cafeteria) {
    if (props.mode === OperationMode.add) {
      const response = await addCafeteria(data);
      setRefreshTrigger((curr) => curr + 1);
      return response;
    } else {
      const response = await updateCafeteria(data, props.initialData?.id as string);
      setRefreshTrigger((curr) => curr + 1);
      return response;
    };
  }
  const onSubmit: SubmitHandler<Cafeteria> = async (data) => {
    const result = handleAddCafeteria(data);
    ...
    <div>
      <Modal isOpen={props.isOpen} onOpenChange={props.onOpenChange} className='font-segoe'>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">{props.mode === OperationMode.add ? "Ajouter" : "Editer"} un Cafeteria</ModalHeader>
              <ModalBody>
                <form className='flex flex-col gap-1 items-start' >
                  <Input variant='bordered' color='secondary' type='text' label="Nom du Cafeteria" {...register('name')} />
                  ...
                  <Select selectionMode='multiple' selectedKeys={selectedKeys} onSelectionChange={setSelectedKeys} color='secondary' label="Employées" className="w-full" {...register('cafeteriaEmployee')}>
...
export default AddCafeteriaModal;