...
import { getRestauMenus } from '@/app/utils/funcs';
...
export default function ManageRestauration({ params }: { params: { id: string } }) {
  ...
  const infos: PageStructureType<SeventhCardItemType, RestauMenu> = {
    ...
    fetchFunc: () => getRestauMenus(params.id)
  }
  return (
    <div>
      <PagesStructure props={infos} url={`/main/employee/restauration/customrestaurant/${params.id}`} Icon={RestauImage} transformData={transformData} />
    </div>
  )
}
